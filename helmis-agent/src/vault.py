...
DATA_DIR = os.environ.get("DATA_DIR") or (
    "/app/data" if os.path.exists("/app/data") else os.path.join(os.path.dirname(os.path.dirname(__file__)), "data")
)
...
def init_vault_structure() -> None:
    ...
    for cat in DEFAULT_CATEGORIES:
        for owner in DEFAULT_OWNERS:
            os.makedirs(os.path.join(VAULT_DIR, cat, owner), exist_ok=True)
...
def save_file_to_vault(
    ...
    if subfolder:
        ...
        dest_dir = os.path.join(VAULT_DIR, clean_subfolder)
    else:
        dest_dir = os.path.join(VAULT_DIR, category_clean, owner_folder)

    if not is_safe_vault_path(dest_dir):
        dest_dir = os.path.join(VAULT_DIR, "documents", owner_folder)
    ...
    rel_path = os.path.relpath(dest_file, VAULT_DIR).replace("\\", "/")
...
def get_vault_file_by_id(file_id: str) -> tuple[dict[str, Any], bytes] | None:
    ...
    for f in catalog.get("files", []):
        if f.get("id") == file_id:
            full_path = os.path.join(VAULT_DIR, f.get("relative_path", ""))
...
def move_vault_files(
    ...
    for record in files:
        ...
        old_full = os.path.join(VAULT_DIR, old_rel)
        ...
        if destination_dir:
            dest_dir = os.path.join(VAULT_DIR, destination_dir.strip("/\\"))
        else:
            dest_dir = os.path.join(VAULT_DIR, cat, owner_folder)
        ...
        new_rel = os.path.relpath(new_full, VAULT_DIR).replace("\\", "/")
...
def create_vault_directory(dir_path: str) -> str:
    ...
    full_path = os.path.join(VAULT_DIR, clean_path)
    if not is_safe_vault_path(full_path):
        raise ValueError(f"Invalid directory path: {dir_path}")
    os.makedirs(full_path, exist_ok=True)
    return os.path.relpath(full_path, VAULT_DIR).replace("\\", "/")
...
def delete_vault_directory(dir_path: str, recursive: bool = False) -> tuple[bool, str]:
    ...
    if full_path == os.path.abspath(VAULT_DIR):
    ...
    catalog = _load_catalog()
    files = catalog.get("files", [])
    rel_prefix = os.path.relpath(full_path, VAULT_DIR).replace("\\", "/")
...
def list_vault_directories() -> list[str]:
    ...
    init_vault_structure()
    dirs: list[str] = []
    for root, dirnames, _ in os.walk(VAULT_DIR):
        for d in dirnames:
            full = os.path.join(root, d)
            rel = os.path.relpath(full, VAULT_DIR).replace("\\", "/")
            dirs.append(rel)
    return sorted(dirs)
